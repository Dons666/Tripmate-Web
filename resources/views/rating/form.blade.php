@if(session('error') || $errors->has('review') || $errors->has('komentar'))
    ...
    <div id="aiWarningModalForm" style="position: fixed; inset: 0; background: rgba(15, 23, 42, 0.75); backdrop-filter: blur(8px); z-index: 9999; display: flex; align-items: center; justify-content: center; padding: 16px;">
        <div style="background: #ffffff; border-radius: 24px; max-width: 440px; width: 100%; padding: 24px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); border: 1px solid #ffe4e6; text-align: center;">
            ...
            <button type="button" onclick="closeAiWarningModalForm()" style="width: 100%; padding: 12px; background: #e11d48; color: #ffffff; font-weight: 800; border-radius: 12px; font-size: 14px; border: none; cursor: pointer; box-shadow: 0 10px 15px -3px rgba(225, 29, 72, 0.3);">
    ...
    <script>
        function closeAiWarningModalForm() {
            var modal = document.getElementById('aiWarningModalForm');
            if (modal) {
