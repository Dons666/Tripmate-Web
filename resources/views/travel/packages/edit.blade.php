        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 sm:p-8">
                    <form action="{{ route('travel.packages.update', $travel->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Nama Paket Wisata</label>
                            <input type="text" name="nama_travel" value="{{ $travel->nama_travel }}" required class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition">
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Deskripsi Singkat</label>
                            <textarea name="deskripsi" rows="3" required class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition">{{ $travel->deskripsi }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="harga_paket" value="{{ $travel->harga_paket }}" required min="0" class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-1">Tanggal Keberangkatan</label>
                                <input type="date" name="tanggal_keberangkatan" value="{{ $travel->tanggal_keberangkatan ? $travel->tanggal_keberangkatan->format('Y-m-d') : '' }}" required min="{{ date('Y-m-d') }}" class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input type="text" name="kota" value="{{ $travel->kota }}" required class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="layanan" value="{{ $travel->layanan }}" required class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-1">Nomor Kontak (WhatsApp)</label>
                                <input type="text" name="kontak" value="{{ $travel->kontak }}" required class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition">
                            </div>
                        </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="armada_id" required class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition">
                                    ...
                                    @foreach($armadas as $armada)
                                        <option value="{{ $armada->id }}" {{ $travel->armada_id == $armada->id ? 'selected' : '' }}>
                                            {{ $armada->nama_kendaraan }} (Kapasitas: {{ $armada->kapasitas_kursi }} Kursi)
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        @php
                            $selectedDest = $travel->destinasis->pluck('id')->toArray();
                        @endphp
                                ...
                                <div>
                                    ...
                                    <select name="wisata_ids[]" multiple class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring focus:ring-sky-200 transition text-sm" style="height: 120px;">
                                        @foreach($wisatas as $dest)
                                            <option value="{{ $dest->id }}" {{ in_array($dest->id, $selectedDest) ? 'selected' : '' }}>{{ $dest->nama_destinasi }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Foto / Poster Paket Saat Ini</label>
                            @if(!empty($travel->gambar))
                                <img src="{{ str_starts_with($travel->gambar, 'http') ? $travel->gambar : asset('storage/' . $travel->gambar) }}" class="w-32 h-32 rounded-xl object-cover border border-slate-200 shadow-sm mb-3">
                            @endif
                        ...
                        <div class="pt-6 border-t border-slate-100 flex gap-3">
                            <button type="submit" class="px-6 py-3 bg-sky-500 hover:bg-sky-600 text-white rounded-xl font-bold transition flex items-center gap-2">
                                💾 Simpan Perubahan
                            </button>
